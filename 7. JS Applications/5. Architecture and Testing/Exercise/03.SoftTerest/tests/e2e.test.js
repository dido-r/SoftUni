...
describe('E2E tests', function () {
  ...
  describe('Catalog', () => {
    ...
    it('show idea details', async () => {
      ...
      const title = await page.textContent('.display-5');
  ...
  describe('Authentication', () => {
    ...
    it('login makes correct API call', async () => {
      ...
      await page.waitForSelector('[class="form-user col-md-8"]');

      await page.fill('[id="inputEmail"]', email);
      await page.fill('[id="inputPassword"]', password);

      const [response] = await Promise.all([
        page.waitForResponse(endpoint),
        page.click('[class="text-center mb-4 text-center"] button'),
      ]);
